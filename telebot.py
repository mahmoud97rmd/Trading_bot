...
def c_log(msg):
    """دالة طباعة السجلات مع الوقت لتتبع أداء البوت"""
...
def calculate_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """حساب المؤشرات الأساسية: المتوسطات، الاستوكاستيك، و ATR"""
    ...
    stoch_k = bot_state.get('stoch_k', 5)       
    stoch_s = bot_state.get('stoch_smooth', 5)    
    stoch_d = bot_state.get('stoch_d', 5)        
...
def get_stoch_signals(prev_k, prev_d, curr_k, curr_d):
    ...
    b_label  = "DEEP(10)" if buy_deep  else "MID(15)"  if buy_mid  else "SHAL(20)" if buy_shal else ""
    s_label  = "DEEP(90)" if sell_deep else "MID(85)"  if sell_mid else "SHAL(80)" if sell_shal else ""
...
def _get_signal_for_bar(df, i, curr, prev, tf):
...
def _strat_label():
    ...
    return "📈 STOCH-NEW" if m == "STOCH_NEW" else "📉 STOCH-OLD"
...
def get_stoch_settings_keyboard():
    ...
    return {"inline_keyboard": [
        ...
        [{"text": "━━ Smooth (S) ━━", "callback_data": "noop"}],
...
async def run_oanda_backtest(start_dt):
    ...
    tol_desc = f"{sm}/{bot_state['filter_mode']}"
    ...
    try:
        for tf in bot_state['timeframes']:
            ...
            for i in df[df['time'] >= start_dt].index:
                ...
                buy_sig, sell_sig, label = _get_signal_for_bar(df, i, curr, prev, tf)
...
# =============================================================
# DIAGNOSTIC ENGINE (STOCHASTIC)
# =============================================================
async def run_diagnostic():
    lines = [f"🔬 <b>تشخيص STOCHASTIC</b>"]
    for tf in bot_state['timeframes']:
        ...
        df = calculate_indicators(pd.DataFrame(c_data).sort_values('time').reset_index(drop=True))
        last = df.iloc[-2]
        lines.append(f"📊 آخر شمعة مغلقة:\n  K={last['K']:.1f}  D={last['D']:.1f}")
        
        buy_sigs, sell_sigs = 0, 0
        for i in range(1, len(df)):
            c = df.iloc[i]; p = df.iloc[i-1]
            b, s, _ = _get_signal_for_bar(df, i, c, p, tf)
            if b: buy_sigs += 1
            if s: sell_sigs += 1
            
        lines.append(f"🎯 إشارات في آخر {len(df)} شمعة: (BUY:{buy_sigs} SELL:{sell_sigs})")
    await send_tg_msg("\n".join(lines))
        ...
        try:
            if bot_state['status'] == 'RUNNING' and bot_state['active_tfs'][tf]:
                ...
                if time_block or danger_now: bot_state['market_data'][tf] = f"⏸ خمول | {df.iloc[-1]['close']:.2f}"
                    ...
                    buy_sig, sell_sig, label = _get_signal_for_bar(df, len(df)-2, curr, prev, tf)
...
async def process_tg_update(update):
    if 'message' in update and 'text' in update['message']:
        ...
        if msg == '/start': 
            await send_tg_msg("🤖 <b>مرحباً! Gold Scalper v3</b>\nتم تحديث البوت والاعتماد على إعدادات الاستوكاستيك.", get_main_keyboard())
        
        elif msg.startswith('/stoch'):
            parts = msg.split()
            if len(parts) == 4:
                try:
                    bot_state['stoch_k'] = int(parts[1])
                    bot_state['stoch_smooth'] = int(parts[2])
                    bot_state['stoch_d'] = int(parts[3])
                    await send_tg_msg(f"✅ تم تغيير إعدادات الاستوكاستيك بنجاح:\nK={bot_state['stoch_k']} | S={bot_state['stoch_smooth']} | D={bot_state['stoch_d']}")
                except ValueError:
                    await send_tg_msg("⚠️ الرجاء إدخال أرقام صحيحة. مثال: /stoch 14 3 3")
            else:
                await send_tg_msg("⚠️ صيغة غير صحيحة.\nاستخدم الأمر بالشكل التالي:\n/stoch [K] [S] [D]\nمثال:\n/stoch 14 3 3")
        ...
        # Fixed Stoch toggles (Deep, Mid, Shal)
        elif d in ("toggle_stoch_deep", "toggle_stoch_mid", "toggle_stoch_shal"):
            attr = d.replace("toggle_stoch_", "use_stoch_")
            bot_state[attr] = not bot_state[attr]
        ...
        # Stoch Settings menu handling
        elif d == "menu_stoch_settings": await edit_tg_msg(chat_id, msg_id, "⚙️ إعدادات الاستوكاستيك:", get_stoch_settings_keyboard())
        elif d == "inc_stoch_k": bot_state['stoch_k'] += 1; await edit_tg_msg(chat_id, msg_id, "⚙️ إعدادات الاستوكاستيك:", get_stoch_settings_keyboard())
        elif d == "dec_stoch_k": bot_state['stoch_k'] = max(1, bot_state['stoch_k'] - 1); await edit_tg_msg(chat_id, msg_id, "⚙️ إعدادات الاستوكاستيك:", get_stoch_settings_keyboard())
        elif d == "inc_stoch_s": bot_state['stoch_smooth'] += 1; await edit_tg_msg(chat_id, msg_id, "⚙️ إعدادات الاستوكاستيك:", get_stoch_settings_keyboard())
        elif d == "dec_stoch_s": bot_state['stoch_smooth'] = max(1, bot_state['stoch_smooth'] - 1); await edit_tg_msg(chat_id, msg_id, "⚙️ إعدادات الاستوكاستيك:", get_stoch_settings_keyboard())
        elif d == "inc_stoch_d": bot_state['stoch_d'] += 1; await edit_tg_msg(chat_id, msg_id, "⚙️ إعدادات الاستوكاستيك:", get_stoch_settings_keyboard())
        elif d == "dec_stoch_d": bot_state['stoch_d'] = max(1, bot_state['stoch_d'] - 1); await edit_tg_msg(chat_id, msg_id, "⚙️ إعدادات الاستوكاستيك:", get_stoch_settings_keyboard())
        elif d == "reset_stoch": bot_state['stoch_k'], bot_state['stoch_smooth'], bot_state['stoch_d'] = 5, 5, 5; await edit_tg_msg(chat_id, msg_id, "⚙️ إعدادات الاستوكاستيك:", get_stoch_settings_keyboard())
        elif d == "preset_14_3_3": bot_state['stoch_k'], bot_state['stoch_smooth'], bot_state['stoch_d'] = 14, 3, 3; await edit_tg_msg(chat_id, msg_id, "⚙️ إعدادات الاستوكاستيك:", get_stoch_settings_keyboard())
...
async def handle_ping(request): return web.Response(text="Gold Scalper Bot v3 — Running Perfect!")
