...
async def run_oanda_backtest(start_dt, end_dt=None):
    ...
    c_log("بدء الباك تيست مع محاكي إدارة المخاطر...")
    csv_filename = f"BT_RiskSim_{datetime.now().strftime('%H%M%S')}.csv"
    trade_logs = []
    
    total_prof, peak_equity = 0.0, 0.0
    max_total_dd_usd, max_total_dd_pct = 0.0, 0.0
    max_daily_dd_usd, max_daily_dd_pct = 0.0, 0.0
    
    # متغيرات المحاكي اليومي
    sim_balance = 10000.0  # حساب وهمي لضبط النسب
    ...
    await send_tg_msg(f"⏳ <b>بدء الباك تيست</b>\n{msg_dt}\nيتم محاكاة التراجع اليومي والخسائر المتتالية...")
    ...
    try:
        for tf in bot_state['timeframes']:
            ...
            for i in df[mask].index:
                ...
                t_date = curr['time'].date()
                
                # تحديث اليوم الجديد
                if current_date_bt != t_date:
                    current_date_bt = t_date
                    daily_start_balance = sim_balance + total_prof
                    consec_losses_bt = 0
                    halted_bt_date = None
                
                if halted_bt_date == t_date:
                    continue  # محظور في هذا اليوم
                ...
                if buy_triggers:
                    for lvl in buy_triggers:
                        ...
                        if can_enter and not entered_this_candle:
                            ...
                            total_prof += p_usd
                            current_equity = sim_balance + total_prof
                            peak_equity = max(peak_equity, current_equity)
                            
                            # حساب التراجع الكلي واليومي
                            current_total_dd_usd = peak_equity - current_equity
                            current_total_dd_pct = current_total_dd_usd / peak_equity if peak_equity > 0 else 0
                            max_total_dd_usd = max(max_total_dd_usd, current_total_dd_usd)
                            max_total_dd_pct = max(max_total_dd_pct, current_total_dd_pct)
                            
                            current_daily_dd_usd = daily_start_balance - current_equity
                            current_daily_dd_pct = current_daily_dd_usd / daily_start_balance if daily_start_balance > 0 else 0
                            max_daily_dd_usd = max(max_daily_dd_usd, current_daily_dd_usd)
                            max_daily_dd_pct = max(max_daily_dd_pct, current_daily_dd_pct)
                            
                            # تطبيق حدود المخاطرة
                            if outcome == "LOSS": consec_losses_bt += 1
                            elif outcome == "WIN": consec_losses_bt = 0
                            
                            note = ""
                            if current_daily_dd_pct >= bot_state['max_daily_dd_pct']:
                                halted_bt_date = t_date
                                note = " [HALTED: Max Daily DD]"
                            elif consec_losses_bt >= bot_state['max_consec_losses']:
                                halted_bt_date = t_date
                                note = " [HALTED: 3 Losses]"
                                
                            trade_logs.append({'Timeframe': tf, 'Type': f'BUY (Lvl {lvl})', 'Entry Time': (curr['time'] + timedelta(hours=3)).strftime('%Y-%m-%d %H:%M'), 'Exit Time': (ext_t + timedelta(hours=3)).strftime('%Y-%m-%d %H:%M'), 'Entry Price': round(act_ent, 2), 'TP': tp_p, 'SL': sl_p, 'Outcome': outcome + note, 'Profit ($)': p_usd})
                        elif not can_enter:
                            trade_logs.append({'Timeframe': tf, 'Type': f'BUY BLOCKED (Lvl {lvl})', 'Entry Time': (curr['time'] + timedelta(hours=3)).strftime('%Y-%m-%d %H:%M'), 'Exit Time': '---', 'Entry Price': curr['close'], 'TP': 0, 'SL': 0, 'Outcome': 'REJECTED (Trend)', 'Profit ($)': 0.0})
                ...
                if sell_triggers and not entered_this_candle:
                    for lvl in sell_triggers:
                        ...
                        if can_enter and not entered_this_candle:
                            ...
                            trade_logs.append({'Timeframe': tf, 'Type': f'SELL (Lvl {lvl})', 'Entry Time': (curr['time'] + timedelta(hours=3)).strftime('%Y-%m-%d %H:%M'), 'Exit Time': (ext_t + timedelta(hours=3)).strftime('%Y-%m-%d %H:%M'), 'Entry Price': round(act_ent, 2), 'TP': tp_p, 'SL': sl_p, 'Outcome': outcome + note, 'Profit ($)': p_usd})
                        elif not can_enter:
                            trade_logs.append({'Timeframe': tf, 'Type': f'SELL BLOCKED (Lvl {lvl})', 'Entry Time': (curr['time'] + timedelta(hours=3)).strftime('%Y-%m-%d %H:%M'), 'Exit Time': '---', 'Entry Price': curr['close'], 'TP': 0, 'SL': 0, 'Outcome': 'REJECTED (Trend)', 'Profit ($)': 0.0})

        if trade_logs:
            df_logs = pd.DataFrame(trade_logs)
            summary_rows = [
                {'Timeframe': '--- SUMMARY ---', 'Type': '', 'Entry Time': '', 'Exit Time': '', 'Entry Price': 'NET PROFIT:', 'TP': '', 'SL': '', 'Outcome': f'${round(total_prof, 2)}', 'Profit ($)': ''},
                {'Timeframe': '', 'Type': '', 'Entry Time': '', 'Exit Time': '', 'Entry Price': 'MAX DAILY DD:', 'TP': '', 'SL': '', 'Outcome': f'${round(max_daily_dd_usd, 2)} ({round(max_daily_dd_pct*100, 2)}%)', 'Profit ($)': ''},
                {'Timeframe': '', 'Type': '', 'Entry Time': '', 'Exit Time': '', 'Entry Price': 'MAX TOTAL DD:', 'TP': '', 'SL': '', 'Outcome': f'${round(max_total_dd_usd, 2)} ({round(max_total_dd_pct*100, 2)}%)', 'Profit ($)': ''}
            ]
            pd.concat([df_logs, pd.DataFrame(summary_rows)]).to_csv(csv_filename, index=False)
            await send_tg_document(csv_filename, f"📊 اكتمل الباك تيست.\nالربح: ${round(total_prof, 2)}\nأقصى تراجع يومي: {round(max_daily_dd_pct*100, 2)}%")
